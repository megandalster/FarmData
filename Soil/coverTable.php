...
include '/Applications/MAMP/htdocs/farmdata/Admin/Delete/warn.php';
?>
<?php
   if (isset($_GET['id'])) {
      ...
      try {
         $stmt = $dbcon->prepare($sqlDeleteMaster);
         $stmt->execute();
      } catch (PDOException $p) {
         phpAlert('', $p);
         die();
   ...
   echo "</table>";
   echo "<br clear=\"all\">";
   $totalAreaSeeded = $dbcon->query("select sum(((Select size from field_GH where fieldID=coverSeed_master.fieldID)/100)*area_seeded) as totalSeeded from coverSeed_master where seedDate BETWEEN '".
         $year."-".$month."-".$day."' AND '".$tcurYear."-".$tcurMonth."-".
         $tcurDay."' and fieldID like '".$fieldID."'");
   
   $rowTotal = $totalAreaSeeded->fetch(PDO::FETCH_ASSOC);
   ...
   echo "</div>";
   $totalByCrop = $dbcon->query("select crop, sum(num_pounds) as total from coverSeed natural join coverSeed_master where seedDate BETWEEN '".
         $year."-".$month."-".$day."' AND '".$tcurYear."-".$tcurMonth."-".
         $tcurDay."' and fieldID like '".$fieldID."' group by crop");
   while($rowCrop = $totalByCrop->fetch(PDO::FETCH_ASSOC)){
      echo "<div class='pure-control-group'>";
