...
if (!$_GET['inst']) {
echo "<form name='form' class='pure-form pure-form-aligned' method='POST' action='/farmdata/down.php'>";
echo "<center>";
...
while($rowM=$resultM->fetch(PDO::FETCH_ASSOC)){
   ...
   echo "<tr><td>".$rowM['sprayDate']."</td><td>".$rowM['fieldID']."</td><td>".
      ...
      "</td><td>".number_format($rowM['frac'],2,'.','')."</td><td>".$rowM['crops']."</td><td>".
   ...
   $masterSQL = "select tSprayMaster.id, sprayDate, waterPerAcre, comment, initials from tSprayMaster ".
     " where sprayDate BETWEEN '".$fromDate."' AND '".$toDate."' and complete=0 and exists ".
     "(select * from tSprayField where tSprayField.id = tSprayMaster.id and tSprayField.crops like '%".
     $crop."%')";
   $result = $dbcon->query($masterSQL);
   while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
      ...
      $watersql = "select sum(numOfBed * size/numberOfBeds) as acres from tSprayField, field_GH where".
        ...
        $fieldCh."'";
      $wRes = $dbcon->query($watersql);
      while ($wRow = $wRes->fetch(PDO::FETCH_ASSOC)) {
